...
Ext.Osiris.RegisterListener("UsingSpell", 5, "after", function(caster, spell,_,_,_)
    ...
    -- print("GENITAL LISTENER ACTIVATED")
    ...
    if containerID == "BG3SX_ChangeGenitals" then
        ...
        -- Check whether entity is shapeshiftet and apply appropriate function
        if (entity.GameObjectVisual.Type == 4) then  -- check if shapeshifted
            print("Entity is shapeshifted")
            
            --Ext.Timer.WaitFor(200, function()
                --Entity:SwitchShapeshiftedVisual(caster, newGenital, "Private Parts")
            Genital:OverrideGenital(newGenital, caster)
            --end)
        
        else 
            print("Entity is normal")
            Genital:OverrideGenital(newGenital, caster)
        end
    end
end)
...
Ext.Osiris.RegisterListener("UsingSpellOnTarget", 6, "after", function(caster, target, spell, _, _, _)
    ...
    --         Genital:GiveErectionToCharacter(caster)
    --        --Osi.UseSpell(caster, "BG3SX_SimpleErections", caster)
    --     end

    --     if (Entity:HasPenis(target)) and (not (target == caster)) then
